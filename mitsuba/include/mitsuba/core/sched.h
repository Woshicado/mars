...
public:
    ...
    virtual ref<WorkResult> createWorkResult(int workerIDX) const = 0;
...
protected:
    ...
    inline void setProcessByID(Item &item, int id) {
        ...
        try {
            ...
            // Log(EInfo, "workerINDEX: %i", item.workerIndex);
            item.workResult = item.wp->createWorkResult(item.workerIndex);
        } catch (std::exception &) {
            throw;
        }
    }
