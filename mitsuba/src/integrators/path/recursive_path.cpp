...
public:
    ...
    bool renderIterationTime(Float until, int &passesRenderedLocal, Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        // int spp = 1 << (m_iteration >> 3);

        bool result = true;
        while (/* passesRenderedLocal < spp && */ true) {
    ...
    bool renderTime(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        int spp;
        Float until = 0;
        for (m_iteration = 0; /* m_iteration < 42 */; m_iteration++) {
            ...
            const bool isPretraining = m_renderingRRSMethod.needsTrainingPhase() && m_iteration < 3;
    ...
    LiOutput Li(LiInput &input, StatsRecursiveValues &stats) const {
        ...
        Assert(input.rRec.depth < 2 || !(rRec.type & RadianceQueryRecord::EEmittedRadiance));
        ...
        for (int comp = 0; comp < bsdf->getComponentCount(); ++comp) {
            roughness = std::min<Float>(roughness, bsdf->getRoughness(its, comp));
        }
        /// we use a simple approximation to convert (assumed) beckmann-roughness to shininess
        const Float shininess = std::max<Float>(0.f, 2.f / (roughness * roughness) - 2.f);
        ...
        if (rRec.depth == 1) {
            ...
            if (samplingNode) {
                ...
                stats.SqError.add(samplingNode->sqError);
            }
        }
    ...
    void computePixelEstimate(const ref<Film> &film, const Scene *scene) {
        ...
        /// For classicRR we need to load a known pixelEstimate to obtain a good efficiency approximation
        if (!m_renderingRRSMethod.needsTrainingPhase()) {
            if (!m_pixelEstimate) {
                const Vector2i size = film->getSize();
                fs::path refPath = scene->getDestinationFile();
                refPath = refPath.parent_path() / "../../ears/out.exr-dbg-EARS.exr";
                auto pxEstimate = new Bitmap(refPath, "pixel.estimate");
                ...
                for (int i = 0; i < size.x * size.y * 3; i++)
                    m_pixelEstimate->getFloat32Data()[i] = pxEstimate->getFloat16Data()[i];

                m_pixelEstimate->write(refPath.parent_path() / "m_pixelestimate.exr");
            }
            return;
        }
...
private:
    ...
    mutable struct ImageStatistics {
        ...
        void reset(Float actualTotalCost) {
            ...
            primary_splits.push_back(m_primarySplit / m_primarySamples);
            ears_factors.push_back({ eF, earsFactorNoReject });
            sq_err.push_back(squareError().average());
...
public:
        std::vector<Float> primary_splits;
        std::vector<Float> costs;
        std::vector<Float> sq_err;
        std::vector<std::pair<Float, Float>> ears_factors;
        int maxPrimarySplit = std::numeric_limits<int>::min();
    } m_imageStatistics;
