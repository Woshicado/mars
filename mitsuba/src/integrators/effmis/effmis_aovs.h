...
struct StatsRecursive {
    ...
    Entry<Float>    numConDensity { "paths.con.density" };
    Entry<Float>    numConPick    { "paths.con.pick"    };
    Entry<Float>    proxyPTMIS    { "proxy.pt.mis"      };
    Entry<Float>    proxyConMIS   { "proxy.con.mis"     };
    /*Stack<Spectrum, 3> rawNEE        { "raw.nee" };
    Stack<Spectrum, 3> rawHit        { "raw.hit" };
    Stack<Spectrum, 3> rawCon        { "raw.con" };
    Stack<Spectrum, 3> weightedNEE   { "w.nee" };
    Stack<Spectrum, 3> weightedHit   { "w.hit" };
    Stack<Spectrum, 3> weightedCon   { "w.con" };
    Stack<Float, 3> misNEE        { "mis.nee" };
    Stack<Float, 3> misHit        { "mis.hit" };
    Stack<Float, 3> misCon        { "mis.con" };
    Stack<Float, 6> misSum        { "mis.sum" };*/
