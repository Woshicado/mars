...
public:
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        fs::path path = scene->getDestinationFile();
        fs::path filename = path.parent_path() / fs::path(formatString("cfg", path.stem().filename().string().c_str()));
        std::ofstream cfgFile(filename.c_str());

        const auto renderTime = timeString(queue->getRenderTime(job), true);

        cfgFile << "RRSMethod:    " << "BDPTRRA"     << '\n'
                << "SPP:          " << sampleCount   << '\n'
                << "Render Time:  " << renderTime    << '\n'
                << "lightImage:   " << (m_config.lightImage ? "true" : "false") << '\n'
                << "Max depth:    " << m_config.maxDepth << '\n'
                << "RR depth:     " << m_config.rrDepth  << '\n'
                << "\n"
                << "------------------------------------------------------------------\n"
                << std::endl;
