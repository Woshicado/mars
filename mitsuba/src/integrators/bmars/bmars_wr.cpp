...
BMARSWorkResult::BMARSWorkResult(const BMARSConfiguration &conf,
...
BMARSWorkResult::~BMARSWorkResult() { }

void BMARSWorkResult::putBlock(const BMARSWorkResult *workResult) {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
void BMARSWorkResult::putLightImage(const BMARSWorkResult *workResult) {
    Assert(m_lightImage);
    m_lightImage->put(workResult->m_lightImage.get());
}

void BMARSWorkResult::clearBlock() {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
void BMARSWorkResult::clearLightImage() {
    Assert(m_lightImage);
    m_lightImage->clear();
}
...
void BMARSWorkResult::dump(const BMARSConfiguration &conf,
...
void BMARSWorkResult::load(Stream *stream) {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
void BMARSWorkResult::save(Stream *stream) const {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
std::string BMARSWorkResult::toString() const {
    return m_block->toString();
}

MTS_IMPLEMENT_CLASS(BMARSWorkResult, false, WorkResult)
MTS_NAMESPACE_END
