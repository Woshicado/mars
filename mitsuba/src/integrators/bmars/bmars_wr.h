...
*/
class BMARSWorkResult : public WorkResult {
public:
    BMARSWorkResult(const BMARSConfiguration &conf, const ReconstructionFilter *filter,
    ...
    void putBlock(const BMARSWorkResult *workResult);
    void putLightImage(const BMARSWorkResult *workResult);

#if BDPT_DEBUG == 1
    /* In debug mode, this function allows to dump the contributions of
       the individual sampling strategies to a series of images */
    void dump(const BMARSConfiguration &conf,
