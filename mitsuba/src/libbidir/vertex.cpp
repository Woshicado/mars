...
bool PathVertex::sampleNext(const Scene *scene, Sampler *sampler,
        ...
        ETransportMode mode, bool russianRoulette, Spectrum *throughput, bool *traced) {
    Ray ray;
    ...
    switch (type) {
        ...
        case ESurfaceInteraction: {
                ...
                // I think I don't want this to be included (because of the splitting factor calculation)
                // May break SOME scenes, but 99+% of scenes will be fine.
...
Spectrum PathVertex::eval(const Scene *scene, const PathVertex *pred,
        const PathVertex *succ, ETransportMode mode, EMeasure measure, bool divTheta) const {
    ...
    switch (type) {
        ...
        case ESurfaceInteraction: {
                ...
                if (divTheta && measure != EDiscrete && Frame::cosTheta(bRec.wo) != 0)
