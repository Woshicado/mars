...
RenderSettingsDialog::RenderSettingsDialog(QWidget *parent) :
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        SLog(EError, "Unable to open the documentation file (%s)",
            file.errorString().toUtf8().constData());
    }

    QString error;
    int line, column;
    if (!m_document.setContent(&file, &error, &line, &column)) {
        SLog(EError, "Unable to read the documentation file (%s in %d:%d)",
            error.toUtf8().constData(), line, column);
    }
    file.close();
